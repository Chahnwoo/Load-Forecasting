...
# - Historical load (CAISO EMS hourly load XLSX library)
# - Derived degree-day features (CDD/HDD) using Fahrenheit temperature

# Key Requirements satisfied:
# - All data collection functions are modular.
# - All data collection functions accept start_date and end_date.
# - All collection is hourly.
# - All timestamps are aligned to UTC so every day has 24 hours (UTC timeline).

# Units (verified from source docs):
# Open-Meteo hourly variables:
#   temperature_2m: °C  -> converted to °F in output
#   apparent_temperature: °C -> converted to °F in output
#   relative_humidity_2m: %
#   precipitation: mm
#   cloud_cover: %
#   wind_speed_10m: km/h
#   shortwave_radiation: W/m²
# CAISO EMS hourly load:
#   MW (kept as MW)

# IMPORTANT UTC NOTE (LOAD):
# CAISO EMS XLSX files are in CAISO market time (America/Los_Angeles) using hour-ending HR.
# When converting to UTC, the first hours of a UTC day may correspond to the prior market day.
# To ensure a complete UTC window with no boundary NaNs, we fetch load for a BUFFERED market window:
#   market_start = start_date - 1 day
#   market_end   = end_date + 1 day
# then convert to UTC and slice to the exact UTC hourly index.

# Station weights:
# - Preferred: read deterministic station points + population weights from:
#     ./data/stations_population_weights.csv
#   generated by build_station_population_weights.py
# - Optional fallback: if CSV missing, you can pass --allow-fallback to use geocoding/fallback coords.
...
# SESSION.headers.update({"User-Agent": "load-forecasting-pipeline/1.0"})
...
#     """
#     HTTP GET -> JSON with:
#       - disk cache (so reruns don't refetch)
#       - respectful handling of 429 (Retry-After + backoff)
...
# #                 params=params,
# #                 timeout=timeout,
# #                 headers={"User-Agent": "load-forecasting-pipeline/1.0"},
# #             )
# #             r.raise_for_status()
# #             return r.json()
# #         except Exception as e:
...
# def http_get_text(url: str, *, timeout: int = 60, max_retries: int = 3) -> str:
#     last_exc = None
#     for attempt in range(max_retries):
#         try:
#             r = requests.get(url, timeout=timeout, headers={"User-Agent": "load-forecasting-pipeline/1.0"})
#             r.raise_for_status()
#             return r.text
#         except Exception as e:
#             last_exc = e
...
#     Returned as tz-naive timestamps that are interpreted as UTC everywhere in this script.
#     """
...
#       - select top max_stations by population_weight for this region
#       - renormalize weights to sum to 1 over the selected subset
#     """
...
#     """
#     Old behavior:
#       - try Open-Meteo geocoding (population may be missing)
#       - else fallback coordinates
#       - if population is missing/zero => uniform weights
#     """
#     _ = (start_date, end_date)
...
#     We request timezone=UTC to ensure 24 hours/day in the time axis.
#     Returns df with:
#       time_utc (UTC-naive hourly timestamp),
#       + WEATHER_HOURLY_VARS
#     """
#     params = {
...
#     """
#     Collect station weather and aggregate into weighted regional weather.
#     Output weather is UTC-aligned hourly with exactly 24 hours/day.
#     Temperature variables are converted to Fahrenheit at the end.
#     """
#     # Prefer deterministic station weights CSV
#     try:
...
#     station_dfs: List[pd.DataFrame] = [fetch_weather_for_station(s, start_date=start_date, end_date=end_date) for s in stations]
...
# # ----------------------------
# # CAISO load collection (market time -> UTC + buffered market window)
# # ----------------------------
...
#         has_date = any(x in row for x in ["date", "opr_dt", "opr date", "trading date"])
#         has_hr = any(x in row for x in ["hr", "hour", "opr_hr", "he"])
...
#     """
#     Interpret naive timestamps as TZ_MARKET, then convert to UTC.
#     Handles DST safely without AmbiguousTimeError by explicitly resolving ambiguous times.
#     """
...
#             TZ_MARKET,
#             ambiguous=True,              # choose DST=True ("first" instance) deterministically
#             nonexistent="shift_forward",
#         )
#     return localized.dt.tz_convert(TZ_UTC).dt.tz_localize(None)

# def parse_caiso_load_xlsx(path: str) -> pd.DataFrame:
#     xls = pd.ExcelFile(path)

#     date_candidates = ["date", "opr_dt", "opr date", "trading date"]
#     hr_candidates = ["hr", "hour", "opr_hr", "he"]
...
#         df[date_col] = _coerce_date_series(df[date_col])
#         df[hr_col] = pd.to_numeric(df[hr_col], errors="coerce").astype("Int64")
#         df = df.dropna(subset=[date_col, hr_col]).copy()

#         market_time = df[date_col] + pd.to_timedelta(df[hr_col].astype(int) - 1, unit="h")
#         df["time_utc"] = _market_hour_start_to_utc_naive(market_time)

#         keep_names = ["pge", "sce", "sdge", "vea", "caiso", "mwd"]
#         out = df[["time_utc"]].copy()
#         for name in keep_names:
#             if name in df.columns:
#                 out[name] = pd.to_numeric(df[name], errors="coerce")

#         load_cols = [c for c in out.columns if c != "time_utc"]
#         if load_cols:
...
#             part = parse_caiso_load_xlsx(path)
#             part = part[(part["time_utc"] >= t0) & (part["time_utc"] <= t1)]
#             if len(part) == 0:
#                 continue
#             parsed_parts.append(part)
#             if debug:
#                 print(f"[caiso_load] parsed OK: {fn} rows={len(part):,} cols={list(part.columns)}")
#         except Exception as e:
#             if debug:
...
#     load = pd.concat(parsed_parts, ignore_index=True)
#     load["time_utc"] = pd.to_datetime(load["time_utc"])
#     load = load.sort_values("time_utc").drop_duplicates(subset=["time_utc"], keep="last")

#     # Reindex to strict UTC window
...
#     allow_fallback: bool,
# ) -> Tuple[pd.DataFrame, pd.DataFrame]:
#     load_wide = collect_caiso_load(start_date=start_date, end_date=end_date, debug=True)
...
#             r, start_date=start_date, end_date=end_date, load_wide=load_wide, allow_fallback=allow_fallback
#         )
#         parts.append(df_r)
#         metas.append(meta_r)
...
#     fn2 = f"station_population_weights.csv"
...
#         "--allow-fallback",
#         action="store_true",
#         help="If station weights CSV is missing/broken, fall back to geocode/fallback coords (not recommended).",
#     )
#     args = ap.parse_args()
